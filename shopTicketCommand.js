  ...
  if (order) {
    ...
    try {
      ...
      const rapportEmbed = new EmbedBuilder()
        ...
        .setTimestamp();
      if (historyUrl) rapportEmbed.addFields({ name: 'Historique du ticket :', value: `[Compte rendu](${historyUrl})`, inline: false });
...
// ── Capture intégrale des messages d'un salon Discord ────────────────────────
...
async function handleDeleteTicket(interaction) {
  ...
  if (orderId) pgStore.saveShopMessages(orderId, capturedMessages).catch(() => {});
