...
function createWebServer(discordClient) {
  ...
  function parseCasinoTransactions(transactions) {
    const casinoSources = ['Casino Roulette', 'Casino Blackjack', 'Casino Slots', 'Casino Roulette Russe', 'Casino Poker'];
    const gameLabels = {
      'Casino Roulette': 'Roulette',
      'Casino Blackjack': 'Blackjack',
      'Casino Slots': 'Slots',
      'Casino Roulette Russe': 'Roulette Russe',
      'Casino Poker': 'Poker',
    };
    return transactions
      .filter(tx => casinoSources.includes(tx.adminId))
      .map(tx => ({ ...tx, game: gameLabels[tx.adminId] || tx.adminId }));
  }
  ...
  app.get('/casino', requireAdmin, async (req, res) => {
    ...
    const { transactions } = await inventoryManager.getTransactions({ limit: 100000 });
    const casinoTxs = parseCasinoTransactions(transactions);
    ...
    casinoTxs.forEach(tx => {
      if (!byGame[tx.game]) byGame[tx.game] = { sessions: 0, wagered: 0, won: 0 };
      byGame[tx.game].sessions++;
      if (tx.quantity < 0) {
        byGame[tx.game].wagered += Math.abs(tx.quantity);
        totalWagered += Math.abs(tx.quantity);
      } else {
        byGame[tx.game].won += tx.quantity;
        totalWon += tx.quantity;
      }
    ...
    const history = casinoTxs.slice(0, 100).map(tx => ({ ...tx, playerName: tx.playerId }));
    if (discordClient) {
      try {
        const guild = discordClient.guilds.cache.first();
        if (guild) {
  ...
  app.post('/casino/config', requireAdmin, async (req, res) => {
    try {
      ...
      const updated = {
        ...
        minBet: {
          roulette: parseInt(req.body.min_roulette) || 10,
          blackjack: parseInt(req.body.min_blackjack) || 10,
          slots: parseInt(req.body.min_slots) || 10,
          russe: parseInt(req.body.min_russe) || 10,
          poker: parseInt(req.body.min_poker) || 10,
        },
        maxBet: {
          roulette: parseInt(req.body.max_roulette) || 10000,
          blackjack: parseInt(req.body.max_blackjack) || 10000,
          slots: parseInt(req.body.max_slots) || 10000,
          russe: parseInt(req.body.max_russe) || 10000,
          poker: parseInt(req.body.max_poker) || 10000,
        },
      };
      await pgStore.setData('casino_config', updated);
      res.redirect('/casino?success=Configuration+casino+enregistrée+!');
    } catch (e) {
